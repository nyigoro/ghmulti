...
function linkAccount(accountName) {
    const workspaceFolder = vscode.workspace.workspaceFolders?.[0].uri.fsPath;
    if (!workspaceFolder) {
        return;
    }
...
async function checkAndPromptForLink() {
    ...
    if (isGitRepo && !isLinked) {
        const choice = await vscode.window.showInformationMessage('This repository isn\'t linked to a ghmulti account. Would you like to link one?', 'Link Account', 'Ignore');
